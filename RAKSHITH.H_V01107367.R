...
setwd('/Users/preethireddy/Downloads/untitled folder')
getwd()
...
hist(kanew$total_consumption, breaks = 10, col = 'blue', border = 'black', 
     xlab = "Consumption", ylab = "Frequency", main = "Consumption Distribution in Karnatka State")

KA_consumption <- aggregate(total_consumption ~ District, data = kanew, sum) 
View(KA_consumption)
??barplot
barplot(KA_consumption$total_consumption, 
        names.arg = KA_consumption$District, 
...
data_map <- st_read("/Users/preethireddy/Desktop/KARNATAKA_DISTRICTS.geojson") 
View(data_map)
